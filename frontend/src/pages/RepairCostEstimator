...
import { estimateRepairCost } from "../api/api";
...
export default function RepairCostEstimator() {
  ...
  async function submit(event) {
    ...
    try {
      const data = await estimateRepairCost({
        ...form,
        userId: user?.id,
    ...
    <main className="container narrow">
      <section className="panel">
        ...
        <form onSubmit={submit} className="grid-form">
          ...
          <select value={form.repairType} onChange={(e) => update("repairType", e.target.value)}>
